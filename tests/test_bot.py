...
def mock_fetch(subject_filter: str | None = None) -> dict:
    """Mock fetcher that returns canned data."""
    if subject_filter is None:
        print(f"  [mock] on_fetch(Check All) → {sum(len(v) for v in MOCK_RESULTS_ALL.values())} recordings")
        return MOCK_RESULTS_ALL
    if subject_filter in MOCK_RESULTS_SINGLE:
        print(f"  [mock] on_fetch('{subject_filter}') → {len(MOCK_RESULTS_SINGLE[subject_filter])} recordings")
        return {subject_filter: MOCK_RESULTS_SINGLE[subject_filter]}
    # Subjects with no recordings
    print(f"  [mock] on_fetch('{subject_filter}') → 0 recordings")
