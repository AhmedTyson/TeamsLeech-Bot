...
class DownloadError(TransferError): pass
class TelegramUploadError(TransferError): pass

class TransferService:
    """
    Handles streaming bytes from Microsoft Graph to local disk, 
    and uploading to Telegram via Pyrogram.
    """
    def __init__(self, graph_client: GraphClient, state_manager: StateManager, tg_client: Client, chat_id: int):
    ...
    def _extract_thumbnail(self, video_path: str) -> Optional[str]:
        ...
        try:
            subprocess.run(
                ["ffmpeg", "-y", "-i", video_path, "-ss", "00:00:02", "-vframes", "1", "-q:v", "2", thumb_path],
    ...
    async def _download_recording(self, rec: Recording, dest_path: str) -> int:
        """Stream recording from Graph API to temp file using httpx."""
        url = f"{GRAPH_BASE_URL}/drives/{rec.drive_id}/items/{rec.item_id}/content"
        
        # We need a new httpx client for streaming so we don't tie up the main GraphClient pool
        async with httpx.AsyncClient() as client:
            try:
                async with client.stream("GET", url, headers=self.graph.headers, timeout=60.0, follow_redirects=True) as resp:
    ...
    async def _upload_to_telegram(self, file_path: str, filename: str, tg_progress_cb: Callable) -> Message:
        ...
        caption = filename[:-4] if filename.lower().endswith(".mp4") else filename
        save_filename = filename if filename.lower().endswith(".mp4") else f"{filename}.mp4"
    ...
    async def upload_recordings(
        ...
        """
        Async Queue Producer/Consumer architecture for high-performance transfers.
        """
        results: List[dict] = []
        ...
        async def _producer():
            for i, rec in enumerate(recordings):
                ...
                try:
                    if progress_cb:
                        await progress_cb("file_progress", {"index": i, "name": rec.name, "percent": 0, "speed_mbps": 0.0})
                    ...
                    if progress_cb: await progress_cb("error", {"index": i, "name": rec.name, "error": str(e)})
                    results.append({"name": rec.name, "success": False, "error": str(e)})
        ...
        async def _consumer():
            while True:
                ...
                i, rec, tmp_path, file_size, start_time_file = item["index"], item["rec"], item["tmp_path"], item["file_size"], item["start_time_file"]
                ...
                async def _tg_progress(current: int, total: int):
                    ...
                    if pct % 5 == 0 and progress_cb:
                        ...
                        speed_mbps = ((current - last_progress_bytes) / (1024*1024)) / elapsed_chunk if elapsed_chunk > 0 else 0.0
                        last_progress_time, last_progress_bytes = now, current
                        await progress_cb("file_progress", {"index": i, "name": rec.name, "percent": pct, "speed_mbps": speed_mbps})

                try:
                    await self._upload_to_telegram(tmp_path, rec.name, _tg_progress)
                    ...
                    try:
                        ...
                        if rec_date > self.state.get_last_run(rec.subject_name):
                            await self.state.save_last_run(rec.subject_name, rec_date)
                            await self.state.save_last_lecture(rec.subject_name, self.state.get_last_lecture(rec.subject_name) + 1)
                    except ValueError:
                        await self.state.save_last_run(rec.subject_name)
                        await self.state.save_last_lecture(rec.subject_name, self.state.get_last_lecture(rec.subject_name) + 1)

                    if progress_cb: await progress_cb("file_done", {"index": i, "name": rec.name, "size_mb": file_size / (1024*1024), "elapsed_s": elapsed_file})
                    results.append({"name": rec.name, "success": True, "error": None})
                except Exception as e:
                    if progress_cb: await progress_cb("error", {"index": i, "name": rec.name, "error": str(e)})
                    results.append({"name": rec.name, "success": False, "error": str(e)})
                finally:
        ...
        await asyncio.gather(asyncio.create_task(_producer()), asyncio.create_task(_consumer()))
